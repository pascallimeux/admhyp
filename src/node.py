...
class Node:
    ...
    def __init__(self, hostname, key_file, type, remoteAdmin):
        ...
        try:
            self.ssh = Ssh(hostname=hostname, username=remoteAdmin, key_file=key_file)
            self.isConnect = True
    ...
    def CreateHyperledgerFolders(self):
        ...
        out, err = self.ssh.ExecCmd("chown "+self.remoteAdmin+"."+self.remoteAdmin+" "+config.GOPATH+"/src/github.com/hyperledger", sudo=True)

    def GetBinaries(self):
        logger.info ("GetBinaries to {}".format(self.hostname))
        self.ssh.UploadFile(config.TGZREPO+config.TGZBINNAME, "/var/hyperledger/"+config.TGZBINNAME)
